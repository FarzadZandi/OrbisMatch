...
class OrbisScraper:
    def __init__(self, args: argparse.Namespace) -> None:
        ...
        self.companies_since_rest_break = 0
        self.next_rest_break_after = random_company_rest_threshold(
            self.args.rest_every_min_companies,
            self.args.rest_every_max_companies,
        )
        ...
        logging.info(
            "Between-company pause range: 12.00-30.00 seconds; distraction pause: 5%% chance of an extra 20-60 seconds; rest break every %s-%s companies for %.0f-%.0f seconds; first rest threshold=%s.",
            self.args.rest_every_min_companies,
            self.args.rest_every_max_companies,
            self.args.rest_min_seconds,
            self.args.rest_max_seconds,
            self.next_rest_break_after,
        )
        ...
        with sync_playwright() as playwright:
            ...
            try:
                ...
                if not self.stop_reason:
                    self.stop_reason = "systemic safety stop"
    ...
    def iterate_reselect_result_pages(self, page: Page, batch: Batch, actual_page_size: int) -> None:
        ...
        while page_number <= max_pages:
                ...
                if not company_name_is_visible_on_current_page(page, row):
                    restored_row = restore_queued_company_results_page(
                        ...
                        after_click=lambda: (
                            ...
                            self.raise_if_systemic_problem(page, "while restoring results page in re-select mode"),
                        ),
                    )
            ...
            advanced = advance_to_next_results_page(
                ...
                after_click=lambda: (
                    self.interaction_pause(),
                    self.raise_if_systemic_problem(page, "after advancing re-select results page"),
                ),
            )
            if not advanced:
                logging.info(
    ...
    def iterate_result_pages(self, page: Page, batch: Batch, actual_page_size: int) -> None:
        ...
        while page_number <= max_pages:
            ...
            advanced = advance_to_next_results_page(
                ...
                self.args.results_timeout_ms,
                before_click=self.interaction_pause,
                after_click=lambda: (self.interaction_pause(), self.raise_if_systemic_problem(page, "after advancing to next results page")),
            )
            if not advanced:
    ...
    def after_company_pause(self) -> None:
        ...
        self.companies_since_rest_break += 1
        if self.companies_since_rest_break < self.next_rest_break_after:
            return

        rest_min = min(self.args.rest_min_seconds, self.args.rest_max_seconds)
        rest_max = max(self.args.rest_min_seconds, self.args.rest_max_seconds)
        rest_seconds = random.uniform(rest_min, rest_max)
        print(
            f"\nRest break due after {self.companies_since_rest_break} companies "
            f"(planned ~{rest_seconds:.0f}s pause to reduce sustained request rate)."
        )
        while True:
            answer = input(
                'Press Enter to take the planned rest, type "c" to continue immediately, '
                "or enter a number of seconds to rest instead: "
            ).strip().lower()
            if answer == "":
                chosen_seconds = rest_seconds
                break
            if answer in {"c", "continue", "skip"}:
                chosen_seconds = 0.0
                break
            try:
                chosen_seconds = float(answer)
                if chosen_seconds < 0:
                    raise ValueError
                break
            except ValueError:
                print('Please press Enter, type "c", or enter a non-negative number of seconds.')
        logging.info(
            "Rest break: user chose to pause %.0f seconds after %s companies (planned was %.0f seconds).",
            chosen_seconds,
            self.companies_since_rest_break,
            rest_seconds,
        )
        if chosen_seconds > 0:
            time.sleep(chosen_seconds)
        self.companies_since_rest_break = 0
        self.next_rest_break_after = random_company_rest_threshold(
            self.args.rest_every_min_companies,
            self.args.rest_every_max_companies,
    ...
    def process_company(self, page: Page, batch: Batch, row: UploadRow) -> int:
        ...
        try:
            ...
            company_row.click(timeout=self.args.default_timeout_ms)
            self.interaction_pause()
            self.raise_if_systemic_problem(page, "after opening company candidate list")
    ...
    def select_winning_candidate_in_orbis(self, page: Page, row: UploadRow, candidate: dict[str, Any]) -> bool:
        ...
        try:
            tray = ensure_candidate_tray(
                ...
                max_pages=self.result_page_search_limit,
                before_page_click=self.interaction_pause,
                after_page_click=lambda: (
                    self.interaction_pause(),
                    self.raise_if_systemic_problem(page, "while re-opening candidate tray on later results page"),
                ),
            )
...
def advance_to_next_results_page(
    ...
    if not wait_for_results_page_settled(page, timeout_ms):
        logging.warning("Current results page did not settle before the page-advance check; not reading transient Next-button state.")
        return False
        ...
        if next_button.count() == 0 or not next_button.is_visible(timeout=1000):
            ...
            if current_page is not None and total_pages is not None and current_page < total_pages:
                ...
                try:
                    ...
                    logging.warning(
                        "Next results-page control did not become visible even though the page indicator shows more pages."
                    )
                    return False
            else:
                return False
    ...
    try:
        ...
        if not wait_for_results_page_settled(page, timeout_ms, previous_names=previous_names):
            logging.warning("Results pagination changed, but the new page did not reach a stable company-name set.")
            return False
    except Exception as exc:
        logging.warning("Could not advance results pagination or detect page change: %s", exc)
        return False
...
def resolve_company_row(page: Page, row: UploadRow) -> ElementHandle | None:
    ...
    if exact_count > 1:
        ...
        logging.warning(
            ...
            "(company=%r, exact-title rows=%s, reference country=%r, reference city=%r).",
            row.own_id,
            row.company,
            exact_count,
